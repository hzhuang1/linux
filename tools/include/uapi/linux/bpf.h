 ...
 *		**-ENOENT** if the bpf_local_storage cannot be found.
 *
 * long bpf_crypto_alloc_shash(const char *alg_name, u32 type, u32 mask, unsigned long *handle)
 *	Description
 ...
 *		Negative value if error occurs.
 *
 * long bpf_crypto_shash_digestsize(unsigned long handle)
 *	Description
 *		Obtain message digest size.
 *	Return
 *		Digest size.
 *
 * long bpf_crypto_shash_setkey(unsigned long tfm, const void *key, size_t key_len)
 *	Description
