...
#else
static inline int crypto_prog_attach(const union bpf_attr *attr,
				     struct bpf_prog *prog)
{
	return -EINVAL;
}

static inline int crypto_prog_detach(const union bpf_attr *attr)
{
	return -EINVAL;
}

static inline int crypto_prog_query(const union bpf_attr *attr,
